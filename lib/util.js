...
function getAssetType(pathname){
    ...
    else if(typeLessSlashes == 'img'){
        var imageType = getImageType(pathname); 
        if(_.indexOf(config.validImageTypes, imageType) !== -1){
            return imageType;
        }
    }
}
...
function getImageType(image){
    ...
    var imageType = image.match(/\.[a-zA-Z]*$/)[0].substring(1);
